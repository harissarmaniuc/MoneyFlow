...
export default function SpendingPage() {
  ...
  const chartData = Object.entries(dailyData)
    .map(([date, amount]) => ({ date, amount, label: format(new Date(date), "MMM d") }))
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card>
          <CardContent className="pt-6">
            ...
            {isLoading ? <Skeleton className="h-7 w-28 mt-1" /> : <p className="text-2xl font-bold">{formatCurrency(totalThisMonth)}</p>}
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
            <p className="text-sm text-muted-foreground">Transactions</p>
            {isLoading ? <Skeleton className="h-7 w-16 mt-1" /> : <p className="text-2xl font-bold">{expenses.length}</p>}
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
            ...
            {isLoading ? (
              ...
              <p className="text-2xl font-bold">{formatCurrency(expenses.length > 0 ? totalThisMonth / new Date().getDate() : 0)}</p>
            )}
          </CardContent>
        </Card>
      </div>
              ...
              <div key={e.id} className="flex items-center justify-between py-2 border-b last:border-0">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {formatDate(e.date)}
                    {e.budgetCategory ? ` · ${e.budgetCategory.name}` : ""}
                  </p>
                </div>
